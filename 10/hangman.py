...
class Hangman(object):
    ...
    def __init__(self, word=None):
        """Setup: get word or pick random one, set a placeholders list ascii only, skip puntuation, init state vars"""
        if word is None:
            word = get_word
    ...
    def _guess_letter(self):
        """Get letter from user and validates it"""
    ...
    def _show_status(self):
        ...
        if self.wrong_letters:
            print('Guessed letters not in word: {}'.format(', '.join(self.wrong_letters)))
    ...
    def play(self):
        ...
        while self.num_wrong_guesses < ALLOWED_GUESSES:
            ...
            if self._has_won():
                print('\nYou won! You guessed {} in {} guesses: '.format(''.join(self.guessed_word), self.num_guesses))
                return
