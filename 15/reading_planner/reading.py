...
def main():
    ...
    query_progress = db.session.query(Task.book_id, db.func.count(db.case([(Task.done, 1)])),
                                      db.func.count(Task.book_id)).group_by(Task.book_id).all()
    progress = {i[0]: i[1]/i[2] * 100 for i in query_progress}
